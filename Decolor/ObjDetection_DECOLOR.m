...
if ~isfield(opt,'lambda'); opt.lambda = 0.2; end % gamma = opt.lambda * beta;
...
beta = 1*(std(Dtau(:,1)))^2.5; % Start from a big value
minbeta = 1*(0.5*std(Dtau(:,1))/2)^2.5; % lower bound: suppose SNR <= 20
